...
export class AnalyticsService {
  static async getDashboardAnalytics() {
    ...
    const [total_audit_logs_7d, total_users, active_users_count, daily_logs, module_stats] =
      await Promise.all([
    ...
    const formatted_modules = module_stats.map((m) => ({
      ...
      percentage:
        total_module_count > 0 ? Math.round((Number(m.count) / total_module_count) * 100) : 0,
    }))
